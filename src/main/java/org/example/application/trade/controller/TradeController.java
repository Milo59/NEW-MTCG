...
// 20 and 21
    ...
    private Response deleteTrade(Request request) {
        ...
        try {
            ...
            trade.setId(request.getPath().replace("/tradings/", "")); //从路径截取id Intercept the id from the path
   ...
   private Response tryToTrade(Request request) {
       ...
       try {
           ...
           if (card2Uid.longValue() == card1Uid.longValue()) { //longValue --> L --> l
               ...
               //判断要拿进来交易的卡 -d card2,满不满足要求 determine whether the card (-d card2) to be brought in for trading, if meet the requirements
               String type; //card2
